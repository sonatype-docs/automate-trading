  ...
  try {
    ...
    switch (job.job_type) {
      ...
      case "execution_engine": {
        const payload = ExecutionEngineJobSchema.parse(job.payload);
        const { runExecutionEngineCore } = await import("@/lib/execution-engine/core.server");
        result = await runExecutionEngineCore(payload);
        break;
      }
