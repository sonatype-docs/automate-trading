  ...
  useEffect(() => {
    ...
    for (const ds of datasets) {
      if (cache.has(ds)) {
        ...
        seedData[ds] = rows;
        seedProg[ds] = { loaded: rows.length, done: true, cached: true };
      } else {
        seedProg[ds] = { loaded: 0, done: false };
      }
    }
    setData(seedData);
      ...
      for (const ds of datasets) {
        ...
        try {
          while (!cancelled && runRef.current === runId) {
            ...
            acc.push(...rows);
            setProgress((p) => ({ ...p, [ds]: { loaded: acc.length, done: false } }));
          ...
          cache.set(ds, acc);
          setData((d) => ({ ...d, [ds]: acc }));
          setProgress((p) => ({ ...p, [ds]: { loaded: acc.length, done: true } }));
        } catch (e) {
          if (!cancelled && runRef.current === runId) setError(e as Error);
          return;
        }
      }
    })();
