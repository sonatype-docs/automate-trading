                  ...
                  <tbody>
                    {[...journal].reverse().map((j, i) => (
                      ...
                      >
                        ...
                        <td
                          className={
                            j.side === "BUY" ? "text-long" : "text-short"
                          }
                        >
                          {j.side || "—"}
                        </td>
                        <td className="text-right">
                          {j.qty.toLocaleString(undefined, {
                            maximumFractionDigits: 6,
                          })}
                        </td>
                        <td className="text-right">
                          {j.price.toLocaleString(undefined, {
                            maximumFractionDigits: 4,
                          })}
                        </td>
                        <td className="text-right text-short">
                          {fmtINR(j.fee, 4)}
                        </td>
                        <td
                          className={`text-right ${j.pnl > 0 ? "text-long" : j.pnl < 0 ? "text-short" : ""}`}
                        >
                          {j.pnl === 0
                            ? "—"
                            : `${j.pnl > 0 ? "+" : ""}${fmtINR(j.pnl)}`}
                        </td>
